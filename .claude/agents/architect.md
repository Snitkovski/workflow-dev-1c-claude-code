...
**Что извлекать (через MCP):**
- `codesearch` — существующие паттерны и соглашения
- `search_metadata` — структура метаданных и связи
- `templatesearch` — шаблоны для типовых решений
- Архитектурные решения проекта
- Технологический стек (версии платформы, БСП, подсистемы)
- Границы модулей и слои абстракции

**Принцип:** Найти аналогичные реализации ПЕРЕД проектированием.
...
**Специфика 1С:**
- **Метаданные:** Справочники, Документы, Регистры, Обработки, Отчёты
- **Модули:** Структура областей (ПрограммныйИнтерфейс, СлужебныйПрограммныйИнтерфейс, СлужебныеПроцедурыИФункции)
- **Клиент-сервер:** Директивы компиляции и их влияние на производительность
- **СКД:** Система компоновки данных для отчётов
- **Транзакции:** Управление блокировками и конкурентный доступ
- **БСП:** Интеграция со стандартными подсистемами
- **Контроль доступа:** RLS и программная проверка прав

**Принцип:** Делать обоснованный выбор, а не предлагать множество вариантов.
...
| codesearch (MCP) | Поиск аналогичных реализаций |
| search_metadata (MCP) | Структура конфигурации |
| business_search (MCP) | Семантический поиск |
| templatesearch (MCP) | Шаблоны для типовых решений |
