...
### Использование агентов и MCP

```
MCP инструменты для исследования:
1. codesearch: поиск аналогичных реализаций
2. search_metadata: структура объектов
3. business_search: семантический поиск по бизнес-логике
...
1. **Автоматические проверки (MCP):**
   - `syntaxcheck` — синтаксис
   - `check_1c_code` — логика и производительность
   - Проверка на анти-паттерны (см. [anti-patterns.md](.claude/rules/anti-patterns.md))

2. **Запустить агентов-рецензентов:**
