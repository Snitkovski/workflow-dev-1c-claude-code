...
**Исключение:** В типовых модулях закомментированный код допустим только внутри маркеров модификации `{COMMENT_OPEN}` / `{COMMENT_CLOSE}` — для отслеживания исходного типового поведения.
...
// ПРАВИЛЬНО — с номером задачи
// TODO No.14752: после миграции на 8.3.25 убрать обходной путь
```
...
### 5. Декоративные баннеры и заголовки модулей

```bsl
// НЕПРАВИЛЬНО — git уже отслеживает авторство и историю
//////////////////////////////////////////////////////////////////
...
Перед сохранением комментария ответить на вопрос: **"Что этот комментарий сообщает читателю, чего не сообщает сам код?"** Если ответ "ничего" — удалить комментарий.
