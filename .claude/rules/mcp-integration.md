...
```
1. search_metadata: [объект для рефакторинга]
   → Получить структуру и зависимости

2. codesearch: [имя функции/объекта]
   → Найти все вызовы (callers) и использования

3. codesearch: [устаревший паттерн]
   → Убедиться, что все вхождения найдены

4. check_1c_code: [новый код]
   → Проверка качества после рефакторинга
```

### При разработке интеграций

```
1. ssl_search: [тема интеграции]
   → Проверить наличие подсистем БСП

2. codesearch: [существующие интеграции]
   → Найти паттерны в текущем коде

3. templatesearch: [HTTP / JSON / XML pattern]
   → Шаблоны для интеграций

4. docsearch: [HTTPConnection / platform API]
   → Проверить API платформы
```
