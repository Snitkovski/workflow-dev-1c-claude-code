...
Данный документ определяет единые стандарты и методологию разработки расширений и конфигураций 1С:Предприятие 8 с использованием AI-ассистента Claude Code. Правила основаны на анализе лучших практик из реальных проектов, сообщества (включая [comol/ai_rules_1c](https://github.com/comol/ai_rules_1c)) и адаптированы для различных сценариев разработки.

## Конфигурация проекта (.dev.env)

**Перед началом работы** прочитать `.dev.env` из корня проекта. Если файла нет — запросить параметры у пользователя. Шаблон: [.dev.env.example](.dev.env.example)

| Параметр | Назначение |
|----------|-----------|
| `{PREFIX}` | Префикс для всех новых объектов метаданных, реквизитов, ролей |
| `{COMPANY}` | Имя компании для комментариев модификации |
| `{DEVELOPER}` | Имя разработчика для комментариев модификации |
| `{PLATFORM_VERSION}` | Версия платформы — определяет доступные возможности |
| `{COMMENT_OPEN}` / `{COMMENT_CLOSE}` | Шаблоны маркеров модификации типового кода |
| `{NEW_OBJECTS_IN}` | Где создавать новые объекты: `main_configuration` или `extension` |
...
- **8.3.18+**: `Асинх` / `Ждать` (для более ранних — `ОписаниеОповещения`)
...
- **Думай сначала** - анализ перед действием
- **Уточняй** - запрашивай детали при неясности

### Принципы разработки

1. **Всегда искать перед написанием** — использовать MCP для поиска шаблонов и существующих решений
2. **Минимальные изменения** — только запрошенное, без scope creep
3. **Двойная проверка** — верифицировать корректность и влияние на зависимости
4. **Чёткая сдача** — резюме изменений, список файлов, риски
...
├── .dev.env                     # Параметры проекта (PREFIX, PLATFORM_VERSION и др.)
├── .dev.env.example             # Шаблон параметров проекта
├── .claude/
│   ├── rules/                   # Правила по категориям
│   │   ├── 1c-code.md          # Стандарты кода 1С
│   │   ├── anti-patterns.md    # Каталог анти-паттернов и code smells
│   │   ├── form-module-rules.md # Правила модулей форм
│   │   ├── platform-solutions.md # Платформенные решения и шаблоны
│   │   ├── integrations.md     # Правила разработки интеграций
...
Анти-паттерны: [.claude/rules/anti-patterns.md](.claude/rules/anti-patterns.md)
Модули форм: [.claude/rules/form-module-rules.md](.claude/rules/form-module-rules.md)
...
| **Тире `—`** | Не использовать — заменять дефисом `-` |
| **Именование** | PascalCase, бизнес-имена, без венгерской нотации |
| **Префикс расширения** | Из `.dev.env` (`{PREFIX}`) или CLAUDE.md (например: `бэкс`, `бтех`) |
| **Длина метода** | ≤ 200 строк (жёсткий), > 100 — ревью |
| **Вложенность** | < 5 уровней |
| **Параметры** | ≤ 5 (дополнительные через Структуру) |

### Critical анти-паттерны (исправлять немедленно)

| Анти-паттерн | Проблема | Решение |
|--------------|----------|---------|
| Запрос в цикле | O(n) запросов | Пакетная обработка `В (&Список)` |
| Точка к реквизитам | Загрузка объекта | `ОбщегоНазначения.ЗначениеРеквизитаОбъекта()` |
| Подзапрос в SELECT | N+1 проблема | JOIN с агрегацией |
| &НаСервере без необходимости | Лишняя сериализация | `&НаСервереБезКонтекста` |
| `Выполнить()`/`Вычислить()` | Безопасность | Явная логика |
| Хардкод учётных данных | Безопасность | БСП / защищённые константы |
| `ТекущаяДата()` на сервере | Рассинхронизация | `ТекущаяДатаСеанса()` |

### Приоритет модификации типового кода
...
1. **Прочитать `.dev.env`** — параметры проекта (PREFIX, PLATFORM_VERSION и др.)
2. **Найти существующие шаблоны** через MCP `templatesearch`
3. **Найти примеры в коде** через MCP `codesearch`
4. **Проверить метаданные** через MCP `search_metadata`
5. **Проверить встроенные функции** через MCP `docsearch`
6. **Проверить функции БСП** через MCP `ssl_search`
7. **Проверить синтаксис** через MCP `syntaxcheck`
8. **Избегать анти-паттернов** — см. [anti-patterns.md](.claude/rules/anti-patterns.md)
9. **Внутреннее саморевью** — стиль, корректность, безопасность, производительность
...
| Категория | Инструмент | Назначение | Язык запроса |
|-----------|------------|-----------|--------------|
| **Поиск** | `templatesearch` | Готовые шаблоны кода | English |
| | `codesearch` | Поиск по коду конфигурации | Русский |
| | `docsearch` | Документация платформы 1С | English |
| | `ssl_search` | Библиотека БСП | Русский |
| | `helpsearch` | Описания функций | Русский |
| **Метаданные** | `search_metadata` | Структура метаданных | Русский |
| | `business_search` | Семантический поиск | Русский |
| | `answer_metadata_question` | Ответы о метаданных | Русский |
| **Качество** | `syntaxcheck` | Проверка синтаксиса | — |
...
- Как реализовано в существующем коде?
...
| [anti-patterns.md](.claude/rules/anti-patterns.md) | **Каталог анти-паттернов и code smells** |
| [form-module-rules.md](.claude/rules/form-module-rules.md) | **Правила модулей форм** |
| [platform-solutions.md](.claude/rules/platform-solutions.md) | **Платформенные решения и шаблоны** |
| [integrations.md](.claude/rules/integrations.md) | **Правила разработки интеграций** |
...
| [performance-optimizer.md](.claude/agents/performance-optimizer.md) | **Оптимизатор производительности** |
...
1. Скопировать структуру `.claude/` из шаблона
2. Создать `.dev.env` из `.dev.env.example` и заполнить
3. Адаптировать `CLAUDE.md` под проект:
   - Указать название и версию
   - Настроить MCP серверы
4. Заполнить `memory-bank/projectbrief.md`
5. Заполнить `memory-bank/techContext.md`
6. Выполнить команду `/start`
...
При использовании этих правил в реальном проекте:

### 1. Создать `.dev.env` из шаблона

```bash
cp .dev.env.example .dev.env
# Заполнить значениями проекта
```

### 2. Добавить в начало CLAUDE.md
...
### 3. Конфигурация для типового кода

Если проект модифицирует типовую конфигурацию — настроить `COMMENT_OPEN`/`COMMENT_CLOSE` в `.dev.env` для маркировки изменений.
