...
1. **Найти существующие шаблоны** через MCP `templatesearch`
2. **Найти примеры в коде** через MCP `codesearch`
3. **Проверить метаданные** через MCP `search_metadata`
4. **Проверить встроенные функции** через MCP `docsearch`
5. **Проверить функции БСП** через MCP `ssl_search`
6. **Проверить синтаксис** через MCP `syntaxcheck`
7. **Избегать анти-паттернов** — см. [anti-patterns.md](.claude/rules/anti-patterns.md)
...
| [anti-patterns.md](.claude/rules/anti-patterns.md) | **Каталог анти-паттернов** |
